...
- **Data acquisition & cleaning** — `data.py` is a standalone pipeline that fetches raw Pokémon and move data from **PokéAPI**, recursively walks evolution-chain graphs (including branching evolutions) to resolve evolution level/trigger, deduplicates moves that appear under multiple learn methods, normalizes inconsistent/missing API fields, and derives fields that aren't given directly (like rarity, computed from capture rate and legendary/mythical status). The cleaned output is cached locally as JSON so the game itself never depends on network access at runtime
- **Data-driven design** — Pokémon, moves, and rarity tables live in JSON (`data/pokemon.json`, `data/moves.json`) rather than being hardcoded, so game content is decoupled from game logic
- **File I/O & serialization** — `to_dict()` / `from_data()` patterns for converting live objects to/from JSON for save/load, and incremental "load, merge, write back" persistence during data fetching
...
- **Loop-based state machines** — leveling loops that handle multiple level-ups (and evolutions) from a single EXP gain in one pass
- **Modular program architecture** — logic is split across single-responsibility modules (`battle.py`, `battle_ai.py`, `encounter.py`, `leveling.py`, `money.py`, `shop.py`, `ui.py`, `data.py`, etc.) rather than one monolithic script
...
### Requirements
- Python 3.x (standard library only — no external dependencies needed to *play*; `data.py` was already run to produce `data/`, so no API calls happen during gameplay)
...
├── data.py                                       # Offline pipeline — fetches & cleans data from PokéAPI into data/
├── data/
│   ├── pokemon.json                                # Cleaned Pokémon stats, types, movesets, evolutions
│   └── moves.json                                    # Cleaned move data (power, effects, etc.)
├── saves/
